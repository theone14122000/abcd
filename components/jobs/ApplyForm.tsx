...
export default function ApplyForm({ job, user }: ApplyFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    console.log("Resume file:", resume ? `${resume.name} (${(resume.size / 1024).toFixed(2)} KB)` : "No file");
    ...
    try {
      ...
      // IMPORTANT: Append resume file if exists
      if (resume) {
        formData.append("resume", resume, resume.name);
        console.log("✅ Resume appended to FormData");
      } else {
        console.warn("⚠️ No resume file attached");
      }
